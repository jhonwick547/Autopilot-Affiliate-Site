...
def render_post(raw_html_path: Path, title: str, market: str, hero_url: str | None) -> str:
    ...
    raw_html_path: Path to the HTML fragment generated by the LLM (content/posts_raw/<slug>.html)
    Returns the path to the rendered file in public/posts/<slug>.html
    """
...
def render_index(posts_info: list):
    """
    Render the index page listing posts.
    posts_info: list of dicts with keys: title, filename, date
    Uses relative links in the template (posts/<filename>) so clicking works on project Pages.
    """
...
def write_basics():
    """Create basic files: .nojekyll and a simple 404 to avoid Jekyll processing issues."""
    PUBLIC.mkdir(parents=True, exist_ok=True)
